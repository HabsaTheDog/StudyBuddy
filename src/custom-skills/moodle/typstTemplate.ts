...
export function studyBuddyTemplatePromptReference(language: SupportedLanguage = "de"): string {
  return [
    ...
    "- #sb-formula(name: ..., variables: (...), units: (...), source: ..., note: ...)[...]",
    "- #sb-math-panel(\"Titel\", note: ...)[...] for complex editable Typst mathematics",
    "- #sb-example(title: ..., result: ..., breakable: false)[...] for worked examples with a distinct task, calculation path, and result. Keep normal examples unbreakable; set breakable: true only when one unusually long example cannot fit on a page.",
    "- Component arguments that contain Typst math must be content blocks, never quoted markup strings. Write `result: [$ x = 2 \"m\" $]`, not `result: \"$x = 2 \\\"m\\\"$\"`. Likewise use content entries such as `variables: ([$bold(r)$: Ortsvektor],)` and `units: ([$bold(r)$: m],)`; otherwise dollar signs and commands are printed literally in the PDF.",
