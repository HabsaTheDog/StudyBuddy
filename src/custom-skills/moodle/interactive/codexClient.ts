...
export function createCodexClient(config: MoodleRuntimeConfig): CodexClient {
  ...
  return {
    async run(prompt, options) {
      ...
      const candidates = selectionAttempts
        .map((selectionAttempt) => ({
          ...resolveCodexModelSelection(config, task, selectionAttempt, options?.operation),
          selectionAttempt,
        }))
        .filter(
          (candidate, index, all) =>
      ...
      for (const [candidateIndex, selection] of candidates.entries()) {
        const policyInput = {
          ...
          attempt: selection.selectionAttempt,
        ...
        const boundedPrompt =
          "Transform only the supplied question/evidence. Do not use tools, skills, shell, files, network or external research. Preserve evidence gaps.\n\n" +
          prompt;
        if (
          boundedPrompt.length + JSON.stringify(options?.outputSchema ?? {}).length >
          resolveModelPromptCharacterBudget(task)
        ) {
          throw new NonRetryableCodexError(
            `${task} request exceeds its prompt/schema budget.`,
            "invalid_request",
          );
        }
        ...
        const metric = {
          id: randomUUID(),
          logicalCallId,
          transportAttempt: candidateIndex + 1,
          task,
          ...
          policySource:
            config.executionProfile || config.modelPolicyOverrides || task === "source_search"
              ? taskModelPolicySource(policyInput)
        ...
        try {
          ...
          const turn = await thread.run(
            options?.imagePaths?.length
              ...
              : boundedPrompt,
            { outputSchema: options?.outputSchema, signal: control.signal },
          );
          usageAvailable = Boolean(turn.usage);
          usage = turn.usage ?? usage;
          toolUsage = summarizeCodexToolUsage(turn.items ?? []);
          control.signal.throwIfAborted();
          if (toolUsage.toolCalls > 0)
            throw new NonRetryableCodexError(
              `${task} leaf worker used ${toolUsage.toolCalls} prohibited tool(s).`,
              "invalid_request",
            );
          status = "completed";
          return turn.finalResponse;
        } catch (error) {
          status = control.timedOut()
            ? "timeout"
            : config.abortSignal?.aborted
              ? "canceled"
              : "failed";
          if (control.timedOut())
            throw new Error(`${task} model call timed out after ${timeoutMs}ms.`, { cause: error });
          const classification = classifyCodexError(error);
          if (
            !config.abortSignal?.aborted &&
            candidates[candidateIndex + 1] &&
            shouldTryModelFallback("failed", classification)
          )
            continue;
          ...
          await appendFile(
            path.join(config.runDir, "run-model-calls.jsonl"),
            JSON.stringify({
              ...metric,
              status,
              usageAvailable,
