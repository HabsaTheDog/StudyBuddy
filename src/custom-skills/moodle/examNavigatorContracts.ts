...
export const CourseChapterSchema = z.object({
  ...
  priority: z.enum(["essential", "important", "supplementary"]).default("important"),
  contentMode: z.enum(["quantitative", "conceptual", "procedural", "case_based", "mixed"]).default("mixed"),
  learningObjectives: z.array(z.string().min(1)).default([]),
  assessmentSignals: z.array(z.string().min(1)).default([]),
