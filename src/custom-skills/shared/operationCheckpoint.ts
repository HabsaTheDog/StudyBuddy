...
import {
  ...
  type ResolveTaskModelPolicyInput,
...
export function operationPolicyFingerprint(
  config: {
    executionProfile?: ResolveTaskModelPolicyInput["profile"];
    codexModel?: string;
    codexReasoningEffort?: ResolveTaskModelPolicyInput["globalReasoningEffort"];
    modelPolicyOverrides?: ResolveTaskModelPolicyInput["overrides"];
  ...
  const input = {
    profile: config.executionProfile ?? "balanced",
    task,
    operation,
  ...
  return semanticHash([
    STUDY_BUDDY_MODEL_POLICY_VERSION,
    ...[1, 2].map((attempt) => resolveTaskModelPolicy({ ...input, attempt })),
  ]);
}
...
export class OperationBudgetError extends NonRetryableCodexError {
  constructor(key: string) {
    super(
      `Operation ${key} exhausted three persisted attempts; preserve the checkpoint for review.`,
      "invalid_request",
    );
  }
}
...
export async function checkpointOperation<T>(input: {
  ...
  const work = previous
    ...
    .then(async () => {
      ...
      for (const root of [
        ...new Set(
          [input.runDir, input.resumeRunDir].filter((value): value is string => Boolean(value)),
        ),
      ]) {
        let raw: string;
        try {
          raw = await readFile(path.join(root, "operation-checkpoints", filename), "utf8");
        } catch (error) {
          if ((error as NodeJS.ErrnoException).code === "ENOENT") continue;
          throw error;
        }
        const record = recordSchema.parse(JSON.parse(raw));
        if (record.key !== input.key || record.bindingHash !== bindingHash)
          throw new Error("Operation checkpoint binding mismatch.");
        attempts = Math.max(attempts, record.attempts);
        previousError ??= record.error;
        if (record.status === "complete" && record.policy === input.policy) {
          if (record.valueHash !== semanticHash(record.value))
            throw new Error("Operation checkpoint content hash mismatch.");
          completed ??= record;
        }
      }
      ...
      const event = async (status: string) =>
        appendFile(
          path.join(input.runDir, "operation-events.jsonl"),
          JSON.stringify({
            key: input.key,
            bindingHash,
            policy: input.policy,
      ...
      while (attempts < 3) {
        ...
        let value: T;
        try {
          value = input.validate(await input.run(attempts, previousError));
          input.signal?.throwIfAborted();
        } catch (error) {
  ...
  try {
    ...
    if (locks.get(destination) === work) locks.delete(destination);
  }
}
