...
import {
  checkpointOperation,
  operationPolicyFingerprint,
  semanticHash,
} from "../shared/operationCheckpoint.js";
import type { CodexClient } from "./codexClient.js";
import {
  studyGuideContentJsonSchema,
...
const decisionSchema = z
  .object({
    mode: z.enum(["author", "delegate"]),
    content: studyGuideContentSchema.nullable(),
    groups: z.array(
      z
        .object({
...
export function validateDelegation(
  groups: Array<{ chapters: number[]; reason: string }>,
  count: number,
): void {
  ...
  if (
    ...
    ids.some((id) => id < 1 || id > count)
  ) {
    throw new Error(
      "Delegation must partition every supplied chapter exactly once into at least two nonempty groups.",
    );
  }
}

/** Fits actual producer prompts, not subject labels or course-size quotas. */
export function boundAuthoringBatches<T>(
  chapters: T[],
  prompt: (group: T[]) => string,
...
export async function authorOrDelegate(input: {
  ...
  contractHash: string;
  buildPrompt: (chapters: AuthoringChapter[]) => string;
  validate: (value: unknown, chapters: AuthoringChapter[]) => StudyGuideContent;
}): Promise<StudyGuideContent> {
  const policy = semanticHash([
    operationPolicyFingerprint(input.config, "learning_content"),
    operationPolicyFingerprint(input.config, "learning_content_repair"),
  ]);
  const binding = {
    version: 1,
  ...
  const base = {
    runDir: input.config.runDir,
    resumeRunDir: input.config.resumeRunDir,
  ...
  const decision = await checkpointOperation<z.infer<typeof decisionSchema>>({
    ...
    validate: (value) => {
      ...
      if (value && typeof value === "object" && !("mode" in value))
        return {
          ...
          content: input.validate(value, input.chapters),
          groups: [],
        };
      const parsed = decisionSchema.parse(value);
      if (parsed.mode === "author") {
        if (parsed.groups.length || !parsed.content)
          throw new Error("Author mode requires content and zero delegates.");
    ...
    run: async (attempt, failure) => {
      const prompt = [
        ...
        input.buildPrompt(input.chapters),
        failure ? `Validation feedback: ${failure}` : "",
      ].join("\n\n");
      return JSON.parse(
        stripFence(
          await input.codex.run(prompt, {
  ...
  await appendFile(
    path.join(input.config.runDir, "authoring-decisions.jsonl"),
    JSON.stringify({
      key: groupKey,
      bindingHash: semanticHash(binding),
      policy,
      mode: decision.mode,
      groups: decision.groups.map((group) => ({
        chapterIndexes: group.chapters.map((number) => input.chapters[number - 1]!.index),
        reason: group.reason,
      })),
  ...
  const workers = await Promise.allSettled(
    Array.from({ length: Math.min(3, decision.groups.length) }, async () => {
      while (next < decision.groups.length) {
        const index = next++;
        const chapters = decision.groups[index]!.chapters.map(
          (number) => input.chapters[number - 1]!,
        );
        results[index] = await checkpointOperation({
          ...base,
          binding: { ...binding, chapters },
          key: `authoring-delegate:${semanticHash(chapters.map((chapter) => chapter.index)).slice(0, 16)}`,
          validate: (value) => input.validate(value, chapters),
          run: async (attempt, failure) =>
            JSON.parse(
              stripFence(
                await input.codex.run(
                  [
                    input.buildPrompt(chapters),
                    failure ? `Validation feedback: ${failure}` : "",
                  ].join("\n\n"),
                  {
                    task: attempt > 1 ? "content_repair" : "content_analyzer",
  ...
  return input.validate(
    {
      ...results[0],
      topics: results.flatMap((result) => result.topics),
